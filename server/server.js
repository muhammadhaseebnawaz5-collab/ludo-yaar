...
io.on("connection", (socket) => {
  ...
  socket.on(
    "invite-friend",
    ({ targetUUID, senderName, senderColor, roomId }) => {
  ...
  socket.on("create-room", ({ name, count, teamUpMode }, callback) => {
    ...
    const room = new LudoRoom(
      roomId,
      { playerCount: count || 4, teamUpMode: !!teamUpMode },
      (event, data) => {
  ...
  socket.on("join-room", ({ roomId, name, sessionId }, callback) => {
    ...
    if (!player)
      return callback({
        success: false,
        error: "Room is full or game in progress",
      });
  ...
  socket.on("voice-signal", ({ roomId, toSocketId, signal }) => {
    socket
      .to(toSocketId)
      .emit("voice-signal", { fromSocketId: socket.id, signal });
  });
